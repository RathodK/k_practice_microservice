package com.kiranrathod.apigateway.config;
...
public class OAuthConfiguration extends WebSecurityConfigurerAdapter {
	...
	/*
	 * @Override public void match(RequestMatchers matchers) {
	 * matchers.anyRequest(); }
	 */
	...
	public void configure(HttpSecurity http) throws Exception {
		http.authorizeRequests()
				...
				.and().csrf().csrfTokenRepository(this.getCSRFTokenRepository()).and()
	 ...
	 * (CSRF) by needing a custom token in the header for any requests that are NOT
	 * safe i.e. modify the resources from the server e.g. POST, PUT & PATCH
	 * etc.<br>
	 * <br>
	 * 
	 ...
	 * follows:
	 * <ol>
	 * <li>Spring security provides the CSRF token value as a request attribute; so
	 * we extract it from there.</li>
	 * <li>If we have the token, Angular wants the cookie name to be "XSRF-TOKEN".
	 * So we add the cookie if it's not there and set the path for the cookie to be
	 * "/" which is root. In more complicated cases, this might have to be the
	 * context root of the api gateway.</li>
	 ...
	 * <code>csrf()</code> filter so that the request attribute for CsrfToken has
	 * been already added before we start to process it.
	 * 
	 * @return
	 */
	...
	/**
	 * Angular sends the CSRF token in a custom header named "X-XSRF-TOKEN" rather
	 * than the default "X-CSRF-TOKEN" that Spring security expects. Hence we are
	 * now telling Spring security to expect the token in the "X-XSRF-TOKEN"
	 * header.<br>
	 * <br>
	 * 
